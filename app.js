...
function renderRaffle() {
  screen.innerHTML = `<h2>Token of Appreciation</h2>
    ...
    <div class="notice">
      <p class="small">
        <strong>The raffle is completely optional.</strong>
        If you do not wish to participate, you may leave the GCash number
        field blank. However, please click <strong>"Submit"</strong> below
        to complete your participation and ensure that your study responses
        are recorded.
      </p>
    </div>
  ...
  bindActions(async () => {
    ...
    // Get the actual Submit button generated by actions()
    const submitButton = document.getElementById("next-btn");

    // Immediately prevent another click.
    if (submitButton) {
      submitButton.disabled = true;
      submitButton.textContent = "Submitting...";
    }

    // Save the GCash number before submitting.
    state.surveyData.gcashNumber =
      document.getElementById("gcashNumber").value.trim();

    // Tell the participant that submission is being processed.
    document.getElementById("raffle-status").textContent =
      "Preparing your responses for submission…";

    try {

      // This is the ONLY submission for a completed participant.
      await submitData();
      ...
      // so the participant can try submitting again.
      if (submitButton) {
